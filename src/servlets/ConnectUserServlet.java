		...
		try{
			...
			if(verified)
				if(User.isBanned(user.getId())){
					...
					response.getWriter().print(new json.Error("Sorry! You are not confirmed!\n"
							+ "Please check kasukasu's confirmation email and confirm your account."));
			else
				response.getWriter().print(new json.Error("Wrong mail or Password"));	
		}
		catch (UserNotFoundException e)
		{response.getWriter().print(new json.Error("Unknown user"));}
	}	 
}